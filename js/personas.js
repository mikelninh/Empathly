...
const GefuehlePersonas = (function () {
  ...
  function renderPersonaPicker(uiLang) {
    ...
    const chooseHint = {
      de: 'Wähle, mit wem du sprechen möchtest',
      en: 'Choose who you want to talk to',
      vi: 'Chọn người bạn muốn trò chuyện',
      el: 'Διάλεξε με ποιον θέλεις να μιλήσεις',
    };

    el.innerHTML = `
      <div class="persona-picker-header">
        <span class="persona-section-title">${sectionTitle[uiLang] || sectionTitle.en}</span>
        <span class="persona-hint">${chooseHint[uiLang] || chooseHint.en}</span>
      </div>
      <div class="persona-cards-row">
        ${PERSONAS.map(p => `
          <button class="persona-card${p.id === active.id ? ' active' : ''}"
            ...
            title="${p.name} — ${p.tagline[uiLang] || p.tagline.en}">
            <span class="persona-avatar">${p.emoji}</span>
            <span class="persona-name">${p.name}</span>
            <span class="persona-lang-label">${p.langLabel}</span>
          </button>`).join('')}
      </div>
      <div class="persona-bio-box">
        <strong class="persona-bio-name">${active.emoji} ${active.name}</strong>
        <span class="persona-bio-text">${active.bio[uiLang] || active.bio.en}</span>
        <span class="persona-bio-specialty">${active.specialty}</span>
      </div>`;
